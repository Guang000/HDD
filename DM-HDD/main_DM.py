        ...
        for it in range(args.Iteration+1):
            ...
            if 'BN' not in args.model: # for ConvNet
                ...
                for c in range(num_classes):
                    ...
                    output_real = embed(img_real).detach().double()
                    output_syn = embed(img_syn).double()
                    ...
                    b1, c1 = x.shape
                    b2, c2 = y.shape
                    x = centroid(k, x).view(c1)
                    y = centroid(k, y).view(c2)
                    loss += man.dist(x, y) * 20
