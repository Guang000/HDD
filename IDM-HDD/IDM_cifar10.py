...
def add_time(space):
    ...
    time = calc_time(space)
    return torch.cat([time, space], dim=-1)

def calc_time(space):
    """ Calculates time component from given space component. """
    return torch.sqrt(torch.norm(space, dim=-1, keepdim=True) ** 2 + k0)

def centroid(k0, x, w=None, eps=1e-8):
    k0 = torch.tensor(k0)
    if w is not None:
        avg = w.matmul(x)
    else:
    ...
    denom = (-math.inner(avg, avg, keepdim=True))
    denom = denom.abs().clamp_min(eps).sqrt()
    centroid = torch.sqrt(k0) * avg / denom
    return centroid
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool[1:]:
                for model_eval in model_eval_pool:
                    ...
                    checkpoint_dir = './checkpoints/{}_ipc_{}_aug_{}_model_{}/'.format(args.dataset, args.ipc, args.aug,
                                                                                       model_eval)
                    ...
                    for image_sign, image_temp in [['syn', image_syn]]:
                        ...
                        for net_ind in range(len(train_net_list)):
                            ...
                            for c in range(num_classes):
                                ...
                                output_real = embed(img_real, last=args.embed_last).detach().double()
                                output_syn = embed(img_syn, last=args.embed_last).double()
                                x = man.expmap0(F.pad(output_real, pad=(1, 0)))
                                x = man.projx(x)
                                y = man.expmap0(F.pad(output_syn, pad=(1, 0)))
                                y = man.projx(y)

                                b1, c1 = x.shape
                                b2, c2 = y.shape
                                x = centroid(k0, x).view(c1)
                                y = centroid(k0, y).view(c2)
                                loss_c += man.dist(x, y) * 100
