...
class TinyImageNetVal(Dataset):
    def __init__(self, root_val, transform=None):
        super().__init__()
        self.root_val = root_val
        self.transform = transform

        anno_file = os.path.join(root_val, "val_annotations.txt")
        self.img2cls = {}
        with open(anno_file, "r") as f:
            for line in f:
                ...
                img_name = parts[0]
                cls_name = parts[1]
                self.img2cls[img_name] = cls_name

        self.images = sorted(os.listdir(os.path.join(root_val, "images")))
        train_root = os.path.join(os.path.dirname(root_val), "train")
        self.cls_list = sorted(os.listdir(train_root))
    ...
    def __getitem__(self, idx):
        img_name = self.images[idx]  # e.g. "val_0.JPEG"
        cls_name = self.img2cls[img_name]
        label = self.cls_to_idx[cls_name]

        img_path = os.path.join(self.root_val, "images", img_name)
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_test = TinyImageNetVal(root_val=val_root, transform=transform)
        class_names = dst_train.classes
